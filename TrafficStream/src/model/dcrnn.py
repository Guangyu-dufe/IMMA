...
class DCRNN(nn.Module, Seq2SeqAttrs):
    ...
    def decoder(self, encoder_hidden_state, adj_mx, labels=None, batches_seen=None):
        ...
        :param adj_mx: adjacency matrix (num_nodes, num_nodes)
        :param labels: (self.horizon, batch_size, num_nodes * output_dim) [optional, not exist for inference]
    ...
    def forward(self, history_data: torch.Tensor, adj_mx: torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None):
        """Feedforward function of DCRNN.

        Args:
            history_data (torch.Tensor): history data with shape [B, L, N, C]
            adj_mx (torch.Tensor): adjacency matrix with shape [N, N]
            future_data (torch.Tensor, optional): future data with shape [B, L, N, C_out]
            batch_seen (int, optional): batches seen till now, used for curriculum learning. Defaults to None.

        Returns:
            torch.Tensor: prediction wit shape [B, L, N, C_out]
        """
        ...
        # reshape data
        batch_size, length, _, channels = history_data.shape
        ...
        if future_data is not None:
            ...
            batch_size, length, _, channels = future_data.shape
        ...
        outputs = outputs.view(B, L, num_nodes,
...
if __name__ == "__main__":
    ...
    # 创建示例数据
    history_data = torch.randn(64, 12, num_nodes, 2).to(torch.device("cuda:7"))
    future_data = torch.randn(64, 12, num_nodes, 1).to(torch.device("cuda:7"))
    adj_mx = adj_mx.to(torch.device("cuda:7"))
    
    # 测试forward
    output = model(history_data, adj_mx, future_data, batch_seen=0)
