...
class GWNET(nn.Module):
    def __init__(self, 
        ...
        if adj_path:
            ...
            self.supports = [torch.tensor(i).to(device) for i in adj_mx]
        else:
            self.supports = None
        ...
        if gcn_bool and addaptadj:
            if aptinit is None:
                ...
                initemb1 = torch.mm(m[:, :node_dim], torch.diag(p[:node_dim] ** 0.5))
                initemb2 = torch.mm(torch.diag(p[:node_dim] ** 0.5), n[:, :node_dim].t())
    ...
    def forward(self, input): # ! (B, C, N, T)
        ...
        if self.gcn_bool and self.addaptadj and self.supports is not None:
            if self.nodevec1 is None or self.nodevec1.size(0) != num_nodes:
                ...
                self.nodevec2 = nn.Parameter(torch.randn(self.node_dim, num_nodes, device=input.device), requires_grad=True)
            
            adp = F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)), dim=1)

            new_supports = []
            for support in self.supports:
                new_supports.append(support.to(input.device))
...
if __name__ == "__main__":
    gwnet = GWNET(device="cuda")
    input = torch.randn(128, 1, 10, 12) # (B, C, N, T)
    output = gwnet(input)
    print(output.shape)