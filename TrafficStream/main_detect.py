    ...
    for year in range(args.begin_year, args.end_year+1):
        ...
        if year > args.begin_year and args.strategy == "incremental":
            ...
            if args.detect:
                ...
                pre_event_severity = np.load(osp.join(args.raw_data_path, str(year-1)+".npz"))["event_severity"]
                cur_event_severity = np.load(osp.join(args.raw_data_path, str(year)+".npz"))["event_severity"]
                # 20% of current graph size will be sampled
                vars(args)["topk"] = int(0.01*args.graph_size) 
                influence_node_list = detect.influence_node_selection(model, args, pre_data, cur_data, pre_graph, cur_graph,pre_event_severity,cur_event_severity)
