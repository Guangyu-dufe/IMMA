        ...
        if year > args.begin_year and args.strategy == "incremental":  # If it is an incremental strategy and the year is greater than the start year
            ...
            if args.detect or args.replay:
                if pre_graph.shape[1] > cur_graph.shape[1]:
                    args.logger.warning("The number of data points in the previous year is greater than the current year, skipping detection and replay.")
                    influence_node_score = np.random.rand(cur_graph.shape[1])
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--first_year_model_path", type = str, default = "log/SD/trafficstream-42/2019/22.3629.pkl", help='specify a pretrained model root')
