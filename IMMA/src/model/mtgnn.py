...
def performance_test():
    """
    测试修复GPU-CPU复制问题后的性能提升
    """
    import time
    
    # 设置设备
    device = torch.device('cuda:7' if torch.cuda.is_available() else 'cpu')
    print(f"性能测试使用设备: {device}")
    
    if device.type == 'cpu':
        print("⚠️  当前使用CPU，无法测试GPU-CPU复制问题的性能影响")
        return
    
    # 模型参数
    num_nodes = 207  # 使用更大的节点数进行测试
    seq_length = 12
    in_dim = 2
    out_dim = 12
    batch_size = 32
    ...
    # 生成测试数据
    input_data = torch.randn(batch_size, seq_length, num_nodes, in_dim).to(device)
    ...
    print("=" * 60)
    print("✅ 修复内容:")
    print("   • torch.zeros() 直接在GPU上创建 (避免CPU→GPU复制)")
    print("   • torch.eye() 直接在GPU上创建 (避免CPU→GPU复制)")
    print("   • 消除了每次前向传播的设备间数据传输开销")
    print("=" * 60)
