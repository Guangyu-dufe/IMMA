...
class GWNET(nn.Module):
    def __init__(self, 
        ...
        if gcn_bool:
            # 简化支持矩阵的处理，不再需要动态adaptive adjacency
    ...
    def forward(self, input,adj): # ! (B, C, N, T)
        ...
        # 使用静态的supports，不再动态创建
        new_supports = None
        if self.supports is not None:
            new_supports = [support.to(input.device) for support in self.supports]
        
        # 如果没有预定义的supports，直接使用输入的adj
        if new_supports is None:
            new_supports = [adj.to(input.device)]
        else:
            # 添加输入的adj到supports中
            new_supports.append(adj.to(input.device))
...
if __name__ == "__main__":
    gwnet = GWNET(device="cuda:7")
    input = torch.randn(4, 1, 8000, 12) # (B, C, N, T)
    adj = torch.randn(8000, 8000)
    output = gwnet(input, adj)
    print(output.shape)