...
def generate_samples(days, savepath, data, event, graph, train_rate=0.6, val_rate=0.2, test_rate=0.2, val_test_mix=False):
    ...
    np.savez(savepath, train_x=train_x, train_y=train_y, val_x=val_x, val_y=val_y, test_x=test_x, test_y=test_y,test_event=test_event, edge_index=edge_index)
    data = {"train_x":train_x, "train_y":train_y, "val_x":val_x, "val_y":val_y, "test_x":test_x, "test_y":test_y,"test_event":test_event, "edge_index":edge_index}
    return data
