...
def main():
    ...
    model = DSTG(**model_config).to(device)
    
    input_data = torch.randn(batch_size, seq_len, num_nodes, input_dim).to(device)
    
    adj_matrix = torch.randn(num_nodes, num_nodes).to(device)
    adj_matrix = [adj_matrix]
    
    output = model(input_data, adj_matrix)
    
    print("输入数据形状:", input_data.shape)
    print("邻接矩阵形状:", adj_matrix[0].shape)
